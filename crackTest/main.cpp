...
void Generate(unsigned int length, string s,string pas)
{
    ...
    if(length == 0) // when length has been reached
    {
        ...
        return;     //sends the compiler back to Crack(), which recurses the algorithm.
    }
...
void Crack(string pass)
{
    ...
    do while(1)
    {
        ...
        if(finalAnswer == pass)
        {
            //Checks the established global variable, which contains a string value created by Generate()-
            //and returns to main() if a match is found. this needs to be modified if the program is applied to a real scenario.
            return;
        }
        stringlength++;
    }
